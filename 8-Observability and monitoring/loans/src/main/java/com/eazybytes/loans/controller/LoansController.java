...
public class LoansController {
    ...
    @GetMapping("/fetch")
    public ResponseEntity<LoansDto>fetchLoan(@RequestHeader(value = "eazybank-correlation-id", required = false) String correlationId,
                                             @RequestParam
                                             @Pattern(regexp="(^$|[0-9]{10})",message = "Mobile number must be 10 digits")
                                             String mobileNumber){
    ...
    public ResponseEntity<LoansContactInfoDto> getContactInfo() {
        logger.info("Invoked contact info method");
        return ResponseEntity
                .status(HttpStatus.OK)
